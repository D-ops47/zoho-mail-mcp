...
async function zohoGet(url, params) {
  ...
  try {
    const res = await axios.get(url, { headers: { Authorization: `Zoho-oauthtoken ${token}` }, params });
    return res.data;
  } catch(e) {
    throw new Error(`Zoho API ${e.response?.status}: ${JSON.stringify(e.response?.data || e.message)}`);
  }
}
...
app.get('/debug', async (req, res) => {
  try {
    const token = await getAccessToken();
    const acctRes = await axios.get('https://mail.zoho.com/api/accounts', { headers: { Authorization: `Zoho-oauthtoken ${token}` } });
    const accounts = unwrap(acctRes.data);
    const accountId = accounts[0]?.accountId;
    let folderTest = null;
    try {
      const fRes = await axios.get(`https://mail.zoho.com/api/accounts/${accountId}/folders`, { headers: { Authorization: `Zoho-oauthtoken ${token}` } });
      folderTest = { status: fRes.status, count: unwrap(fRes.data).length };
    } catch(fe) { folderTest = { error: fe.response?.status, body: JSON.stringify(fe.response?.data).substring(0,200) }; }
...
app.post('/mcp', async (req, res) => {
  ...
  try {
    ...
    if (method === 'tools/list') return ok({ tools: [
      ...
      { name: 'get_email', description: 'Get full content of an email', inputSchema: { type: 'object', properties: { accountId: { type: 'string' }, messageId: { type: 'string' } }, required: ['accountId','messageId'] } },
    ...
    if (method === 'tools/call') {
      ...
      if (name === 'list_accounts') {
        const d = await zohoGet('https://mail.zoho.com/api/accounts');
        const accounts = unwrap(d).map(a => ({ id: a.accountId, email: a.primaryEmailAddress || a.incomingUserName, name: a.displayName || a.accountDisplayName, primary: a.isDefaultAccount }));
        return ok({ content: [{ type: 'text', text: JSON.stringify(accounts, null, 2) }] });
      }

      if (name === 'list_folders') {
        const d = await zohoGet(`https://mail.zoho.com/api/accounts/${args.accountId}/folders`);
        const folders = unwrap(d).map(f => ({ id: f.folderId, name: f.folderName, path: f.path || f.folderPath, unread: f.unreadCount, total: f.messageCount }));
        return ok({ content: [{ type: 'text', text: JSON.stringify(folders, null, 2) }] });
      }

      if (name === 'list_emails') {
        const d = await zohoGet(`https://mail.zoho.com/api/accounts/${args.accountId}/messages/view`, { folderId: args.folderId, limit: args.limit || 20, start: args.start || 1 });
        const emails = unwrap(d).map(m => ({ id: m.messageId, subject: m.subject, from: m.fromAddress || m.sender, to: m.toAddress, date: m.receivedTime || m.sentTime, hasAttachment: m.hasAttachment, summary: m.summary }));
        return ok({ content: [{ type: 'text', text: JSON.stringify(emails, null, 2) }] });
      }

      if (name === 'search_emails') {
        const d = await zohoGet(`https://mail.zoho.com/api/accounts/${args.accountId}/messages/search`, { searchKey: args.query, start: 0, limit: args.limit || 20 });
      ...
      if (name === 'get_email') {
        // Correct Zoho single-message endpoint: /api/accounts/{id}/messages/{msgId}
        const d = await zohoGet(`https://mail.zoho.com/api/accounts/${args.accountId}/messages/${args.messageId}`);
        const msg = d.data || d;
        return ok({ content: [{ type: 'text', text: JSON.stringify(msg, null, 2) }] });
      }

      if (name === 'list_attachments') {
        // Attachments are in the message details response
        const d = await zohoGet(`https://mail.zoho.com/api/accounts/${args.accountId}/messages/${args.messageId}`);
        const msg = d.data || d;
        const attachments = (msg.attachments || msg.attachment || []).map(a => ({ id: a.attachmentId || a.id, name: a.attachmentName || a.name, size: a.attachmentSize || a.size, contentType: a.attachmentType || a.mimeType }));
